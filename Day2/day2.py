...
def id_sum(games):
    ...
    for i in range(len(lines)):
        if is_possible(lines[i].split(":")[1]):
            total += i+1
    return total

print("Sum of the IDs for example is", id_sum(example_input))
print("Sum of the IDs for real is", id_sum(real_input))
...
'''
def min_set_power(game):
    min_num = {'red' : 0, 'blue' : 0, 'green' : 0}
    sets = game.split(";")
    for s in sets:
        cc = cube_count(s)
        for color in cc:
            if cc[color] > min_num[color]:
                min_num[color] = cc[color]
...
def min_power_sum(games):
    ...
    lines = games.split('\n')
    for i in range(len(lines)):
        total += min_set_power(lines[i].split(":")[1])
    return total

print("Min number set power sum for example is", min_power_sum(example_input))
print("Min number set power sum for example is", min_power_sum(real_input))
