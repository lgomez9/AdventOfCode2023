...
def find_total(winning_cards, cards):
    total = 0
    for card in cards:
...
def card_winnings(card):
    values = card.split(':')[1]
    winning_nums, owned = values.split('|')
    winning_cards = set()
    for num in winning_nums.strip().split(' '):
        if num != '':
            winning_cards.add(num)
    owned_cards = owned.strip().split(' ')
    return find_total(winning_cards, owned_cards)

def get_winnings(cards):
    total = 0
    for card in cards.split('\n'):
        winnings = card_winnings(card)
        total += winnings
    return total

print("The total winnings of the example input is", get_winnings(example_input))
print("The total winnings of the real input is", get_winnings(real_input))
...
'''
def number_of_matches(winning_cards, cards):
    total = 0
    for card in cards:
        if card == '':
            continue
...
def get_scratchcards(cards):
    ...
    for i in range(len(cards)):
        card = cards[i]
        values = card.split(':')[1]
        nums, owned = values.split('|')
        winning_cards = set()
        for num in nums.strip().split(' '):
            if num != '':
                winning_cards.add(num)
        owned_cards = []
        for o in owned.strip().split(' '):
            if o != '':
                owned_cards.append(o)
        match_count = number_of_matches(winning_cards, owned_cards)
...
def total_scratchcards(cards):
    return get_scratchcards(cards.split('\n'))
    

print("The total scratchcards of the example input is", total_scratchcards(example_input))
print("The total scratchcards of the real input is", total_scratchcards(real_input))